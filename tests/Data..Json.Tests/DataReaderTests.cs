...
public class DataReaderTests
{
    ...
    public void ExecuteReader_Basic()
    {
        using (var conn = new JsonConnection($"Data Source=./Sources/database.json;"))
        {
            ...
            using (var reader = cmd.ExecuteReader())
            {
                ...
                Assert.True(success);
                ValidateRow(reader, "Joe", "Joe@gmail.com", 56000L, true);

                //Read second row
                success = reader.Read();
                Assert.True(success);
                ValidateRow(reader, "Bob", "bob32@gmail.com", 95000, null);
    ...
    private void ValidateRow(IDataReader reader, string expectedName, string expectedEmail, 
                             long expectedSalary, bool? expectedMarried)
    {
