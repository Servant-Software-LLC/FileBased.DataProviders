...
public class CsvCommandTests
{
    ...
    public void ExecuteScalar_ShouldReturnFirstRowFirstColumn()
    {
        ...
        connection.Open();
        var command = new CsvCommand(query, connection);
        ...
        var scalarCommand = new CsvCommand(query, connection);
    ...
    public void ExecuteScalar_ShouldCountRecords()
    {
        ...
        var command = new CsvCommand("SELECT COUNT(*) FROM locations where id=1 or id=2", connection);
        ...
        command = new CsvCommand("SELECT COUNT(*) FROM employees where name='Joe' OR name='Bob' OR name='Jim' OR name='Mike'", connection);
